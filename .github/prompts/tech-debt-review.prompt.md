...
argument-hint: "Optional: commit count or date range to scope the scan"
agent: "architect"
---
...
2. Diff the scope (`git diff <base> -- modules/goblin/`) and scan changed files for debt signals:
   ...
   - Naming violations: `Goblin*` / `goblin_*` on classes, methods, files (policy: `.github/copilot-instructions.md` Naming)
...
- Hard rules in `.github/copilot-instructions.md` apply.
