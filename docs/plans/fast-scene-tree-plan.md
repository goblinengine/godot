# FastSceneTree â€” Implementation Plan
...
> module-era plan below (Â§2â€“Â§10) is kept for history only.
> **Acceptance = suite green + no regressions + smoke boots** (A/B benchmark
> baseline and per-project opt-out are lost vs the module design â€” accepted).

- **Spec (superseded):** `docs/rfc/fast-scene-tree-rfc.md`
- **Backlog:** M-14
- **Edit home:** `modules/goblin/scene/main/scene_tree.cpp` + direct core edit
  `scene/main/scene_tree.h` (user-sanctioned: 7 lines â€” 2 ProcessGroup flags
...
### Batch 1 â€” T1: copy-free process iteration
...
redundant â€” exit-tree null-marks before `node_removed`). Live-list iteration
with captured count + per-iteration re-read + null-skip.

**Files:** `scene/main/scene_tree.h` (+2 ProcessGroup flags) + mirror.

### Batch 2 â€” T6: copy-free group calls + timer ref efficiency
...
`ptrw()` â€” old code force-detached the copy on **every** group call (full
...
per timer per frame â€” matches `process_tweens`). Cached signal names
...
- 4000-node churn stress: group counts exact (2000/6000), survivors process
  20 frames, disabled stop at 5, re-added process 11 frames â€” all PASS
- Editor/PM/game boots clean; byte-identical script output
- Timing print: 1100 â†’ 416 usec (~62% faster)
...
fork â€” dispatched calls reach `GDScriptFunction::call` (err=0) but
...
touch other files or have observable behavior effects â€” deferred unless a
narrower sanction is granted:

| Tier / M-item | What | Why deferred |
|---|---|---|
| **T2** | Intrusive group lists, direct Callable dispatch, incremental order | Needs `node.h` (per-node group membership links) â€” out of tree-only scope |
| **T3** | Ordered unique-group-call flush | Replaced `_flush_ugc` HashMap order â€” same copy cost as T6 already fixed |
| **T4** | Subtree pause/suspend mode flags (skip paused subtrees wholesale) | Needs node-side `Tree` data (process mode/pause state) â€” `node.h` edit |
| **M4** | Incremental children cache (no full dump+sort on remove) | `node.cpp:1771` â€” not `scene_tree.cpp` |
| **M5** | Coalesced `tree_changed()` (dirty flag, emit once at frame end) | Signal timing is observable â€” behavior-visible change |
...
## Original plan (history â€” module-era design, NOT followed)
...
### Phases (historical â€” module era)
...
| P5 | Optimizations (T1â€“T6, M4â€“M7) | Behavior identical |
...
- node.h scene edit was upstream-touch â€” now **not needed** (in-place approach
  only touches scene_tree.{cpp,h}).
- Re-implementation semantics drift â€” **not a concern** (in-place edits, no
  new contract to match).
- Type-identity divergence â€” **not a concern** (SceneTree still IS the tree).
- Scope â€” reduced to copy-free iteration + name caching (no full re-implementation).
