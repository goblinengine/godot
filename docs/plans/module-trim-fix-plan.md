# Module Trim Fix â€” Implementation Breakdown & Analysis
...
> `docs/plans/module-trim-fix-plan.md`. Backlog B-01 links here.
...
2. Replace the dead `env.disabled_modules` code in `config.py` (no-op + latent setâ†’list crash).
3. Re-validate the trim list against evidence, because every "verified unused" claim in ADR 0003
   was gathered while the trim was a no-op â€” the evidence is **unvalidated**.
...
- `modules/goblin/config.py:244-248` appends `DISABLE_MODULES` into `env.disabled_modules` â€”
...
| `detect_modules` | `files.sort()` â†’ `modules_detected` ALPHABETICAL | methods.py:276 |
| First module loop | per module `import config` â†’ `is_enabled()` â†’ `opts.Add(BoolVariable("module_x_enabled", ...))`; **goblin imports between `godot_physics_3d` and `gridmap`** | SConstruct:471-494 |
| `opts.Update(env, {**ARGUMENTS, **env.Dictionary()})` | The ONE Update that applies `module_*_enabled` (first application; `env.Dictionary()` has no module keys yet â†’ **ARGUMENTS rules**) | SConstruct:499 |
...
## 3. Locked mechanism: **A â€” import-time ARGUMENTS injection** (ADR 0012)

All inside `modules/goblin/config.py`. At goblin's `import config` in the first module loop
(SConstruct:474 â€” unconditionally, enabled or not), module-level code runs:
...
custom.py/profile files per SCons 4.10.1 `Variables.Update` semantics) â†’ `BoolVariable`
converts `"no"` â†’ `False` â†’ `env["module_x_enabled"] = False` â†’ gate at 1113 skips **all 28
regardless of alphabetical position**.

Precedence rule (locked):
1. **User CLI** (`module_x_enabled=yes/no`) â€” already in `ARGUMENTS` at goblin import â†’ guard
   skips â†’ user wins. This is how `module_mono_enabled=no` already works in the canonical
   build command.
2. **Injection** (our `"no"`) â€” applies when the user said nothing.
3. **custom.py / profile files are beaten** by the injection (args layer > files layer).
   Accepted today: no repo-root `custom.py` exists (verified). Documented limitation â€” if a
...
- **B â€” repo-root `custom.py`**: new file outside `modules/goblin/` (hard rule 2 requires
  explicit permission), a second source of truth that can drift from `DISABLE_MODULES`,
  and still beaten by ARGUMENTS anyway.
- **C â€” `get_opts()` duplicate `BoolVariable`s defaulting False**: covers all 28, but
  duplicates option keys (Help() pollution, option-system abuse).
- **D â€” configure()-time env flags only**: insufficient alone (10/28 unreachable), and
...
(`config.py:244-248`) â€” removes the no-op AND the latent setâ†’list crash
(`methods.py:358` `self.disabled_modules.add`).

Regression canary: `configure()` gains a verification print â€” if injection regressed
(e.g. upstream renames the options path), `env[f"module_{m}_enabled"]` reads `True` and the
count printed is < 28 â†’ visible in every build log.

New mechanism â†’ documented in `.kilo/rules/rules.md` (mechanism 4) + the `overrides` skill
...
```python
# ===================================================================
# MODULE TRIM â€” 28 modules disabled (~55% faster compile)
...
# that Update (gate loop, SConstruct:1124) â€” too late for the 10 trim
...
# ===================================================================
DISABLE_MODULES = {
    # Image/texture formats (PNG only) â€” tinyexr KEPT (editor .exr
    ...
    # glslang â€” Forward+/Mobile unsupported by design, ADR 0003)
    "glslang",
    # Utility (no webcam, no runtime zip)
    "camera", "zip",
    # Rendering (Embree occlusion culling â€” Forward+/Mobile only)
    "raycast",
...
|---|---|---|
| `tinyexr` | **REMOVE from trim (re-enable)** | `Image::save_exr` is `ERR_UNAVAILABLE` without `save_exr_func` (image.cpp:2824-2829), registered only by `modules/tinyexr` (register_types.cpp:46). Editor lightmap bake save writes `.exr` (lightmap_gi.cpp:894) â€” C-11, already decided in lightmapper-cpu-plan Â§9b.3c. Trim never engaged â†’ the break was latent; engaging the trim without this would break editor lightmap saves. |
| `godot_physics_3d` | **REMOVE from trim (re-enable)** | New evidence (2026-08-16): jolt_physics registers "Jolt Physics" but NOT as default (jolt_physics/register_types.cpp:59 â€” `register_server` only). Default server comes from `godot_physics_3d/register_types.cpp:56-57` (`set_default_server("GodotPhysics3D")`). `main.cpp:362-372`: `new_server(GLOBAL_GET("physics/3d/physics_engine"))` â†’ `new_default_server()` â†’ dummy fallback + warning. `tests/test_main.cpp:204`: `[SceneTree]` tests call `new_default_server()` directly â†’ dummy â†’ C-14 combat tests (11 doctest cases) fail. The editor override pins `physics/3d/physics_engine = "Jolt Physics"` for new projects (goblin editor_node.cpp:8344), so Jolt stays the project-selected engine; GodotPhysics3D stays as the always-registered default (boot + tests). |
| `godot_physics_2d` | **KEEP trimmed** | 2D physics â†’ `PhysicsServer2DDummy` (main.cpp:390-392, graceful). No GDScript-suite 2D physics usage (verified: tests/scripts has no PhysicsBody2D/Area2D); `test_viewport.cpp:1601` explicitly handles the dummy cast. Reference title is 3D. **Gate:** full suite must stay green (P3); if any doctest needs real 2D physics â†’ re-enable with ADR 0003 evidence. |
| `glslang` | **KEEP trimmed** | `RenderingDevice::shader_compile_spirv_from_source` (rendering_device.cpp:230-238) fails without glslang â†’ Forward+/Mobile runtime broken by design. Fork is GL Compatibility-only (ROADMAP, C-12/C-13, lightmap decisions all assume GLES3). `#ifdef MODULE_GLSLANG_ENABLED` guards are the only uses (rendering_device.cpp:48/230, rendering_device_binds.cpp:34/200) â€” nothing else in the build depends on it. **Gate:** editor boots + renders on GLES3 (P3); document "Forward+/Mobile require re-enabling glslang" in ADR 0003 + ROADMAP. |

Net: **30 â†’ 28 trimmed modules.**
...
visual_shader, jolt_physics, midi, combat, gdscript, ...) â€” none requires a trimmed module.
...
| P2 | Build gates: editor build + trim-absence proof + log check | none (verification) | 0.5-1d | Â§7 gates 1-4 |
| P3 | Runtime gates: suite (tests=yes), editor smoke, GLES3 render, PNG import (D-07), .exr save (C-11) | none (verification) | 0.5-1d | Â§7 gates 5-7 |
| P4 | Reference-title validation (unvalidated-evidence gate): project loads, corpus imports, gameplay smoke | none (verification) | 1d | Â§7 gate 8; any break â†’ re-enable module with ADR 0003 evidence |
| P5 | Docs: ROADMAP Â§5 mechanism + Impact risk line + counts + GL-compat-only note | `docs/ROADMAP.md`, CODE_MAP note if needed | 0.5d | doc review (architect) |

Architect artifacts (proposed, pending write access â€” full text in the handoff message):
...
1. **Editor build green**: `scons platform=windows target=editor module_mono_enabled=no
   accesskit=no angle=no debug_symbols=yes -j4` (build skill) â†’ binary + PDB present.
2. **Trim provably absent from compile**:
   - `modules/modules_enabled.gen.h` (generated from `env.module_list`, modules_builders.py:8-12)
     defines `MODULE_<X>_ENABLED` for gated modules only â†’ grep: no `MODULE_BMP_ENABLED` etc.
   - `bin/obj/modules/` â†’ no `module_<trimmed>` library/objects.
   - `scons --tree=all -n bin/goblin.windows.editor.x86_64.exe` â†’ grep `modules/<trimmed>` â†’
...
4. **Canary counter-example** (optional, once): pass `module_bmp_enabled=yes` â†’ log shows
   27/28 + bmp compiles (proves user-CLI precedence AND the gate both work).
5. **Suite green**: `tests=yes` build, full doctest run (1397/1397 today on the no-op build â€”
...
6. **Editor smoke on GLES3**: headless + windowed boot, scene open/save, PNG texture import
   produces `.import` sidecar (D-07), lightmap bake save path writes `.exr` (C-11 â€” tinyexr).
...
8. **Corpus import pass**: all reference-title assets import with only PNG/kept formats
   ...
   trimmed module â†’ re-enable that module (bmp/tga/jpg/webp/hdr/dds) with evidence.

---

## 8. Risks & contingencies

- **Suite breaks when trim engages** â€” the 1397/1397 was measured on the no-op build; the
...
- **Corpus has non-PNG assets** â€” import breaks when trim engages; contingency: re-enable the
  serving module (ADR 0003 evidence).
- **Forward+/Mobile users** â€” broken at runtime without glslang (RD shader compile
  `ERR_FAIL`). Accepted: fork is GL-compat-only; documented in ADR 0003 + ROADMAP.
  Contingency: re-enable glslang.
- **Editor import pipeline with compression modules disabled (D-07)** â€” PNG path uses
...
- **custom.py / profile re-enable beaten by injection** â€” documented limitation, none exists
  today; CLI remains the sanctioned override.
- **Help() shows stale defaults** (trimmed modules display `yes` default while injected `no`
  wins) â€” cosmetic, accepted (C would fix it but pollutes the option system).
- **Upstream drift in the options flow** (opts.Update/ARGUMENTS path changes) â†’ injection
  silently no-ops â†’ the configure() canary print (count < 28) catches it.
- Accepted Risk (from user brief): the trim engaging may break the 2D-physics test surface â€”
  consequence: possible suite patch or godot_physics_2d re-enable â€” why proceeding: the fork
  is 3D-genre; dummy fallback is upstream-graceful; evidence gates decide.

## 9. Rollback path

- **Whole fix**: `git revert` the single-file `config.py` change â†’ trim returns to today's
  no-op behavior (builds still work). Zero upstream files touched â†’ no rebase impact.
- **Single module**: remove it from `DISABLE_MODULES`, rebuild â€” one-line, per-module.
...
- Whether the fork should also gate the 2D-physics dummy behavior in docs (project settings
  hint) â€” follow-up doc task, not this plan.
- `lightmapper_rd` stays compiled but is unusable on RD (no glslang) and disabled on GL-compat
  (no RD) â€” dead weight until the lightmapper_cpu plan disables it (C-02 phase 1.4). Not this
