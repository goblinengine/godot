# Goblin Engine â€” FastSceneTree RFC
...
> seam, no retype ripple â€” the optimized tree runs everywhere (editor, PM, games)
> for free. The module-era design (Â§2â€“Â§10 below) is kept for history only.
>
> - **Companion plan:** `docs/plans/fast-scene-tree-plan.md`
> - **Backlog:** M-14
...
change â€” same public API, same signals, same semantics, same process-group
...
- A `BaseSceneTree` core-header seam + `node.h` retype (~326 call sites, ~40 friend
  accesses, ~30 engine files) â€” retype ripple the architect judged too broad.
...
itself â€” the private `ProcessGroup`/`group_map` machinery is accessible, no
...
### T1 â€” Copy-free process iteration
...
(provably redundant â€” exit-tree ordering null-marks before `node_removed`).

**Files:** `scene/main/scene_tree.h` (+2 `ProcessGroup` flags) + goblin mirror.

### T6 â€” Copy-free group calls + timer efficiency
...
(`Ref<SceneTreeTimer> &timer`) â€” no refcount churn per timer per frame (matches
...
|---|---|---|
| **T2** | Intrusive group lists, direct Callable dispatch, incremental order | Needs `node.h` (per-node group links) â€” upstream touch beyond scene_tree |
| T3 | Ordered unique-group-call flush (`_flush_ugc`) | Same copy cost as T6 â€” already fixed |
...
| **M4** | Incremental children cache (no full dump+sort on remove) | `node.cpp:1771` â€” not scene_tree.cpp |
| **M5** | Coalesced `tree_changed()` (dirty flag, emit once at frame end) | Signal timing is observable â€” behavior-visible |
| **M6** | Iterative `_propagate_*` (flat worklist vs recursive O(subtree)) | `node.cpp:595` propagation â€” not scene_tree.cpp |
...
order, group-call flags, timer modes, pause/suspend, scene change â€” all
identical to upstream. Verified via:
- Doctest suite: 1337/1337 passed, 0 failed, 1 skipped
- GDScript suite: 516 assertions âœ“ | Completion: 635 âœ“ | LSP: 57,556 âœ“
- 4000-node churn stress: group counts exact (2000/6000), survivors keep
  processing, disabled stop, re-added process â€” all invariants PASS
- Editor/PM/game boots; byte-identical script output; timing 1100â†’416 usec
...
3. **T5 cadence API:** folds into SimServer S-01 â€” not needed for SceneTree itself.

---

## 8. Original module-era design (history â€” NOT followed)
...
- New class `FastSceneTree : public MainLoop` â€” zero ClassDB conflict, selected
  per-project via `application/run/main_loop_type`.
- Full re-implementation of the SceneTree contract â€” same public API, same
  signals, same semantics â€” new internals: no per-frame process-list copies,
...
- Extending `SceneTree` inherits the machinery we need to replace:
  ...
  and `ProcessGroup`/`group_map` are private non-virtual â€” a subclass can only
  add parallel systems, never replace internals.
- `BaseSceneTree : MainLoop` core-header seam + `node.h` retype â€” ~326 get_tree()
...
```
BaseSceneTree : MainLoop           â† NEW additive core header: scene/main/base_scene_tree.h
â”œâ”€â”€ SceneTree : BaseSceneTree     â† narrow upstream edit
â””â”€â”€ FastSceneTree : BaseSceneTree â† module implementation
```

- `BaseSceneTree` in core as `scene/main/base_scene_tree.h` (header-only, pure
  virtuals â€” core header cannot include a module header).
- `node.h` edit: `data.tree` / `_set_tree` / `get_tree()` â†’ `BaseSceneTree *`.
...
|---|---|---|
| M1 | per-frame process vector copy (~40KB/5k nodes, 60Ã—/s) | zero-copy epoch iteration (T1) |
